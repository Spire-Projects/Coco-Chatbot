...
export const PromptEditorPanel = ({ sessionId }: PromptEditorPanelProps) => {
  ...
  const canPublishSession = useMemo(() => Boolean(sessionPrompt.trim()), [sessionPrompt]);

  const loadPromptData = async () => {
    setIsLoading(true);
    try {
      const [globalCurrent, sessionCurrent] = await Promise.all([
        api.getGlobalPrompt(),
        api.getSessionPrompt(sessionId)
      ]);

      setGlobalPrompt(globalCurrent.content);
      setSessionPrompt(sessionCurrent.content);
      setMessage("");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "No se pudieron cargar los prompts.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const publishSession = async () => {
    ...
    try {
      const sessionSaved = await api.updateSessionPrompt(sessionId, sessionPrompt.trim());
      await loadPromptData();
      setMessage(`Prompt de sesion ${sessionId} actualizado (${sessionSaved.updatedAt}).`);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "No se pudo actualizar el prompt de sesion.");
    } finally {
    ...
    <section className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-8 shadow-xl relative overflow-hidden mt-2">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 relative z-10 border-b border-white/10 pb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-2">Edita el prompt global y el especifico de la sesion seleccionada, luego publica para aplicar cambios.</p>
        </div>
        ...
        <div className="flex gap-3 flex-wrap">
          <button 
            ...
            disabled={isLoading || isPublishingGlobal || isPublishingSession}
          >
          ...
          <button 
            ...
            disabled={!canPublishGlobal || isLoading || isPublishingGlobal || isPublishingSession}
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg>
            Guardar global
          </button>
          <button
            className="px-5 py-2.5 bg-fuchsia-600 hover:bg-fuchsia-500 text-white shadow-lg shadow-fuchsia-500/20 rounded-xl font-medium transition-all hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center gap-2 text-sm"
            onClick={publishSession}
            disabled={!canPublishSession || isLoading || isPublishingGlobal || isPublishingSession}
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg>
            Guardar sesion
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
        <label className="flex flex-col gap-3">
          ...
          <textarea
            className="w-full bg-slate-900/50 border border-white/10 text-slate-300 rounded-2xl p-4 focus:outline-none focus:ring-2 focus:ring-fuchsia-500/50 focus:border-fuchsia-500/50 transition-all font-mono text-sm leading-relaxed resize-y min-h-[160px] shadow-inner"
            value={globalPrompt}
            onChange={(event) => setGlobalPrompt(event.target.value)}
            rows={8}
            placeholder="Introduce las instrucciones generales que aplican a todos los bots..."
          />
        </label>
        ...
        <label className="flex flex-col gap-3">
          ...
          <textarea
            ...
            rows={8}
            placeholder="Instrucciones especificas para este vendedor..."
          />
        </label>
      </div>
